...
import type { CodeFile } from './store.js'
...
function recordView(r: CodeFile): Record<string, unknown> {
  return {
    id: r.id,
    name: r.name,
    lang: r.lang,
...
/** 记录 → 轻量轮询视图。 */
function metaView(r: CodeFile): Record<string, unknown> {
  return {
    id: r.id,
    name: r.name,
    lang: r.lang,
      ...
      async handler(req: any, res: any) {
        ...
        try {
          ...
          if (pathname === '/dsh-code-workbench/ai/chat' && req.method === 'POST') {
            ...
            if (messages.length === 0 || messages.some((m: any) => !m || typeof m.content !== 'string')) {
            ...
            const reply = await chatCompletions({
              ...
              apiKey,
              model,
              messages: messages.map((m: any) => ({ role: String(m.role ?? 'user'), content: String(m.content) })),
          ...
          if (pathname === '/dsh-code-workbench/ai/apply' && req.method === 'POST') {
            ...
            const applied = result.applied.map((m) => {
              ...
              if (!record) return { fileId: m.fileId, name: m.fileId, ok: false, error: '文件不存在' }
