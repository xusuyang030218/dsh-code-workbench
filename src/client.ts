/**
 * 浏览器端（TypeScript）— 代码工作台 v3.1
 *
 * 布局：顶栏 / 文件栏（可拖拽，文件夹导入）/ 编辑区（Tab + 四视图 + 行号编辑）/
 *       AI 助手（直连 DeepSeek 等已配置 API，可选模型，直接对话/修改）/
 *       命令栏（操作 chips + 动态提交，宿主 agent 流程）
 *
 * 零运行时依赖：语法高亮自写 token 化器、diff 自写 LCS（行级+词级）、CSS 内联注入。
 * 由 tsc 编译为 lib/client.js（保持 window.__ModuleLoader__.load 契约）。
 */
declare function require(id: string): any

;(window as any).__ModuleLoader__.load({
  id: 'dsh-code-workbench',
  factory: (requireFn: any) => {
    const module = { exports: {} as any }
    const exports = module.exports
    const React: typeof import('react') = requireFn('react')
    ...
    const getOpen = () => open
    const setOpen = (v: boolean) => { open = v; listeners.forEach((fn) => fn()) }
    ...
    const ACTION_LABEL: Record<string, string> = ACTIONS.reduce((m, a) => { m[a.id] = a.label; return m }, {} as Record<string, string>)
    ...
    const SOURCE_LABEL: Record<string, string> = { upload: '上传', paste: '粘贴', ai: 'AI', manual: '手动编辑', rollback: '回滚' }
    ...
    const GENERIC_SPEC: LangSpec = { line: '//', block: ['/*', '*/'], strings: ['"', "'"], triple: false, html: false, keywords: GENERIC_KW }
    ...
    function tokenize(code: string, spec: LangSpec): Token[] {
      ...
      let i = 0
      let buf = ''
      const flush = (type: string) => { if (buf) { tokens.push({ type, text: buf }); buf = '' } }
      while (i < n) {
    ...
    function renderHighlighted(code: string, lang: string): any[] {
    ...
    // ===== LCS 核心 =====
    interface LcsOp { type: 'same' | 'del' | 'add'; aIndex?: number; bIndex?: number }
    ...
    interface DiffRow { type: 'same' | 'mod' | 'del' | 'add' | 'folded'; left?: string | null; right?: string | null; lnL?: number | null; lnR?: number | null; count?: number }

    function lineDiffRows(aText: string, bText: string): DiffRow[] {
    ...
    interface Span { type: string; text: string }

    function wordDiffSpans(leftLine: string, rightLine: string): { left: Span[]; right: Span[] } {
      ...
      const left: Span[] = []
      const right: Span[] = []
      const merge = (arr: Span[], type: string, text: string) => {
        if (!text) return
      ...
      for (const op of ops) {
        if (op.type === 'same') { merge(left, 'same', a[op.aIndex!]); merge(right, 'same', b[op.bIndex!]) }
        else if (op.type === 'del') merge(left, 'del', a[op.aIndex!])
        else merge(right, 'add', b[op.bIndex!])
      }
      return { left, right }
    }

    function foldSame(rows: DiffRow[], ctx = 3, threshold = 6): DiffRow[] {
      const out: DiffRow[] = []
      let i = 0
      const n = rows.length
      while (i < n) {
        if (rows[i].type === 'same') {
          let j = i
          while (j < n && rows[j].type === 'same') j += 1
          const len = j - i
          if (len > threshold) {
            for (let k = i; k < Math.min(i + ctx, j); k++) out.push(rows[k])
            out.push({ type: 'folded', count: len - 2 * ctx })
            for (let k = Math.max(j - ctx, i); k < j; k++) out.push(rows[k])
          } else {
    ...
    function diffStats(rows: DiffRow[]): { add: number; del: number } {
      let add = 0
      let del = 0
      for (const r of rows) {
        if (r.type === 'add') add += 1
        else if (r.type === 'del') del += 1
        else if (r.type === 'mod') { add += 1; del += 1 }
      }
      return { add, del }
    }
    ...
    function parseInline(text: string): any[] {
      ...
      while ((m = re.exec(text))) {
        ...
        const tok = m[0]
        if (tok.startsWith('**')) parts.push(React.createElement('strong', { key: 'b' + key }, tok.slice(2, -2)))
        else if (tok.startsWith('`')) parts.push(React.createElement('code', { key: 'c' + key, className: 'cw-mdc' }, tok.slice(1, -1)))
        else if (tok.startsWith('[')) parts.push(React.createElement('a', { key: 'a' + key, href: m[3], target: '_blank', rel: 'noreferrer' }, m[2]))
        else parts.push(React.createElement('em', { key: 'e' + key }, tok.slice(1, -1)))
        last = re.lastIndex
        key += 1
      }
    ...
    function renderMarkdown(md: string): any[] {
      ...
      const flushList = () => {
        ...
        out.push(React.createElement(listType === 'ol' ? 'ol' : 'ul', { key: 'l' + key, style: { margin: '4px 0', paddingLeft: 22 } },
          listBuf.map((li, idx) => React.createElement('li', { key: idx, style: { marginBottom: 2 } }, li))))
        listBuf = []
        listType = null
      }
      const flushTable = () => {
        ...
        out.push(React.createElement('table', { key: 't' + key, style: { borderCollapse: 'collapse', fontSize: 12, margin: '6px 0' } },
          React.createElement('thead', null, React.createElement('tr', null, head.map((c: any, i: number) => React.createElement('th', { key: i, style: { border: '1px solid rgba(128,128,128,.25)', padding: '3px 8px', textAlign: 'left' } }, c)))),
          React.createElement('tbody', null, tableBuf.slice(1).map((row: any[], ri: number) => React.createElement('tr', { key: ri }, row.map((c, ci) => React.createElement('td', { key: ci, style: { border: '1px solid rgba(128,128,128,.25)', padding: '3px 8px' } }, c)))))))
        tableBuf = []
      }
      for (const raw of lines) {
        ...
        if (inCode) {
          if (/^\s*(```|~~~)/.test(line)) {
            inCode = false
            out.push(React.createElement('pre', { key: 'pc' + key, className: 'cw-code', style: { background: 'rgba(128,128,128,.07)', border: '1px solid rgba(128,128,128,.18)', borderRadius: 8, padding: 8, overflow: 'auto', fontSize: 12, lineHeight: '18px', margin: '4px 0' } },
              React.createElement('code', { style: { fontFamily: 'inherit' } }, codeBuf.join('\n'))))
            codeBuf = []
          } else {
            codeBuf.push(line)
          }
        ...
        if (/^\s*\|.*\|\s*$/.test(line) && line.replace(/^\s*\|/, '').replace(/\|\s*$/, '').includes('|')) {
          ...
          if (isSepRow) { flushTable(); continue }
    ...
    const btn = (label: any, onClick: (() => void) | undefined, opts: any): any => {
      opts = opts || {}
      return React.createElement('button', {
        ...
        title: opts.title,
        style: Object.assign(
          { border: 'none', borderRadius: 7, padding: '4px 10px', cursor: 'pointer', fontSize: 11.5, lineHeight: '18px', whiteSpace: 'nowrap' },
          opts.primary
            ? { background: '#2563eb', color: '#fff', fontWeight: 600 }
            : { background: 'transparent', border: '1px solid rgba(128,128,128,.35)', color: 'var(--dsw-alias-label-primary, #1e293b)' },
          opts.active ? { background: 'rgba(37,99,235,.1)', borderColor: 'rgba(37,99,235,.45)', color: '#2563eb', fontWeight: 600 } : {},
          opts.ghost ? { borderColor: 'transparent', background: 'transparent', color: 'var(--dsw-alias-label-secondary, #8493ab)' } : {},
          opts.disabled ? { opacity: .5, cursor: 'not-allowed' } : {},
          opts.style || {},
        ),
      }, label)
    }
    const hint = (text: string) => React.createElement('div', { style: { color: 'var(--dsw-alias-label-secondary, #8493ab)', fontSize: 12, padding: '10px 4px' } }, text)

    // ===== 输入栏按钮 =====
    function CodeButton(): any {
      const isOpen = React.useSyncExternalStore(subscribe, getOpen)
      const toggle = (e: any) => { e.preventDefault(); e.stopPropagation(); setOpen(!isOpen) }
      return React.createElement('button', {
        type: 'button',
    ...
    // ===== 行号高亮代码块 =====
    function LineCode(props: any): any {
      const lines = splitLines(props.code)
      return React.createElement('div', { className: 'cw-scroll cw-code', style: { overflow: 'auto', maxHeight: props.maxHeight || 340, background: 'rgba(128,128,128,.05)', border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, margin: '6px 0' } },
        lines.map((line, i) => React.createElement('div', { key: i, style: { display: 'flex', padding: '0 10px', lineHeight: '18px' } },
          React.createElement('span', { style: { color: 'rgba(128,128,128,.6)', minWidth: '2.4em', textAlign: 'right', paddingRight: 10, userSelect: 'none', flexShrink: 0 } }, String(i + 1)),
          React.createElement('span', { style: { whiteSpace: 'pre', flex: 1, minWidth: 0, overflow: 'hidden' } }, line === '' ? ' ' : renderHighlighted(line, props.lang)),
        )))
    }

    // ===== 源码视图（行号 + 高亮 + 编辑态叠加） =====
    function CodeView(props: any): any {
      ...
      const header = React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderBottom: '1px solid rgba(128,128,128,.15)', fontSize: 11.5 } },
        React.createElement('span', { style: { color: 'var(--dsw-alias-label-secondary, #8493ab)' } }, editing ? '编辑中…' : '源码'),
        React.createElement('span', { style: { flex: 1 } }),
        !editing && btn('⧉ 复制', onCopy, { ghost: true, style: { padding: '2px 8px' } }),
        !editing && btn('复制带行号', onCopyLines, { ghost: true, style: { padding: '2px 8px' } }),
        !editing && btn('⬇ 下载', onDownload, { style: { padding: '2px 8px' } }),
        editing
          ? React.createElement('span', { style: { display: 'flex', gap: 6 } },
              btn('取消', onCancel, { style: { padding: '2px 8px' } }),
              btn('保存为新版本', onSave, { primary: true, style: { padding: '2px 8px' } }))
          : btn('编辑', () => props.onStartEdit(), { style: { padding: '2px 8px' } }))
      if (!editing) {
        return React.createElement('div', { style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, background: 'rgba(128,128,128,.05)', overflow: 'hidden' } },
          header,
          React.createElement(LineCode, { code, lang: file.lang, maxHeight: props.maxHeight || 360 }))
      }
      return React.createElement('div', { style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, background: 'rgba(128,128,128,.05)', overflow: 'hidden' } },
        header,
        React.createElement('div', { style: { display: 'flex', maxHeight: props.maxHeight || 360 } },
          React.createElement('div', { ref: lnRef, className: 'cw-scroll', style: { width: 44, overflow: 'hidden', flexShrink: 0, background: 'rgba(128,128,128,.04)', borderRight: '1px solid rgba(128,128,128,.12)', padding: '8px 0', fontSize: 12, lineHeight: '18px', textAlign: 'right', color: 'rgba(128,128,128,.6)', userSelect: 'none' } },
            lines.map((_: string, i: number) => React.createElement('div', { key: i, style: { paddingRight: 8 } }, String(i + 1)))),
          React.createElement('div', { style: { position: 'relative', flex: 1, minWidth: 0 } },
            React.createElement('pre', { ref: preRef, className: 'cw-code', style: Object.assign({}, base, { position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', color: 'var(--dsw-alias-label-primary, #1e293b)' }), 'aria-hidden': 'true' },
              React.createElement('code', { style: { fontFamily: 'inherit' } }, lines.map((line, i) => React.createElement('span', { key: i }, line === '' ? ' ' : renderHighlighted(line, file.lang), '\n')))),
    ...
    function DiffView(props: any): any {
      const { leftText, rightText, mode, showFold } = props
      const [expanded, setExpanded] = React.useState(new Set<number>())
      const rows = lineDiffRows(leftText, rightText)
      const folded = showFold ? foldSame(rows) : rows
      const stats = diffStats(rows)
      const foldKeyOf = (idx: number) => { let key = 0; for (let i = 0; i < idx; i++) if (folded[i].type === 'folded') key += 1; return key }
      const rowBg = (type: string) => DIFF_BG[type] || 'transparent'
      const wordCache: Record<string, { left: Span[]; right: Span[] }> = {}
      const getWordSpans = (l: string, r: string) => {
        const k = l + '\u0001' + r
        if (!wordCache[k]) wordCache[k] = wordDiffSpans(l, r)
        return wordCache[k]
      }
      ...
      const foldRow = (r: any, i: number) => {
        const k = foldKeyOf(i)
        return React.createElement('div', { key: 'f' + i, className: 'cw-diff-fold' },
          React.createElement('span', { style: { flex: 1, borderTop: '1px dashed rgba(128,128,128,.35)' } }),
          React.createElement('button', {
            type: 'button',
            onClick: () => setExpanded((prev: Set<number>) => { const s = new Set(prev); if (s.has(k)) s.delete(k); else s.add(k); return s }),
          ...
          }, expanded.has(k) ? '收起' : ('⋯ ' + r.count + ' 行未变更 · 点击展开 ⋯')),
          React.createElement('span', { style: { flex: 1, borderTop: '1px dashed rgba(128,128,128,.35)' } }))
      }
      const modSpans = (r: any, side: string) => {
      ...
      const simpleCell = (text: string, ln: number | null | undefined, type: string) => React.createElement('span', null,
        React.createElement('span', { style: lnStyle }, ln || ''),
        React.createElement('span', null, (type === 'del' ? '− ' : type === 'add' ? '+ ' : '') + text))

      if (mode === 'split') {
        return React.createElement('div', { className: 'cw-diff', style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, overflow: 'hidden' } },
          React.createElement('div', { style: { display: 'flex', background: 'rgba(128,128,128,.05)', borderBottom: '1px solid rgba(128,128,128,.15)', fontSize: 11.5, fontWeight: 600, color: 'var(--dsw-alias-label-secondary, #8493ab)' } },
            React.createElement('div', { style: { flex: 1, padding: '4px 12px' } }, '原始代码'),
            React.createElement('div', { style: { flex: 1, padding: '4px 12px', borderLeft: '1px solid rgba(128,128,128,.15)' } }, '修改后代码')),
          React.createElement('div', { className: 'cw-scroll', style: { maxHeight: props.maxHeight || 380, overflow: 'auto' } },
            folded.map((r, i) => {
              if (r.type === 'folded') return foldRow(r, i)
              ...
              return React.createElement('div', { key: i, style: { display: 'flex', background: rowBg(r.type) } },
                React.createElement('div', { style: { flex: 1, minWidth: 0, padding: '1px 6px 1px 12px', whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, left),
                React.createElement('div', { style: { flex: 1, minWidth: 0, padding: '1px 6px 1px 12px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', borderLeft: '1px solid rgba(128,128,128,.12)' } }, right))
            })))
      }
      return React.createElement('div', { className: 'cw-diff', style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, overflow: 'hidden' } },
        React.createElement('div', { className: 'cw-scroll', style: { maxHeight: props.maxHeight || 380, overflow: 'auto', padding: '4px 0' } },
          folded.map((r, i) => {
            if (r.type === 'folded') return foldRow(r, i)
            if (r.type === 'mod') {
              return React.createElement('div', { key: i },
                React.createElement('div', { style: { display: 'flex', padding: '1px 12px', background: rowBg('del'), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
                  React.createElement('span', { style: lnStyle }, r.lnL || ''),
                  React.createElement('span', { style: { color: '#dc2626', fontWeight: 700, paddingRight: 6 } }, '−'),
                  React.createElement('span', { style: { flex: 1 } }, modSpans(r, 'left'))),
                React.createElement('div', { style: { display: 'flex', padding: '1px 12px', background: rowBg('add'), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
                  React.createElement('span', { style: lnStyle }, r.lnR || ''),
                  React.createElement('span', { style: { color: '#16a34a', fontWeight: 700, paddingRight: 6 } }, '+'),
                  React.createElement('span', { style: { flex: 1 } }, modSpans(r, 'right'))))
            }
            ...
            return React.createElement('div', { key: i, style: { display: 'flex', padding: '1px 12px', background: rowBg(r.type), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
              React.createElement('span', { style: lnStyle }, ln || ''),
              prefix,
              React.createElement('span', { style: { flex: 1 } }, content))
          })))
    }

    // ===== 历史视图 =====
    function HistoryView(props: any): any {
      ...
      return React.createElement('div', { style: { fontSize: 12 } },
        React.createElement('div', { style: { fontWeight: 700, fontSize: 13, marginBottom: 8 } }, file.name + ' · 版本历史'),
        React.createElement('div', { style: { position: 'relative', paddingLeft: 26 } },
          React.createElement('div', { style: { position: 'absolute', left: 7, top: 10, bottom: 10, width: 2, background: 'rgba(128,128,128,.2)' } }),
          versions.map((v: any) => {
            const isCur = v.v === current
            return React.createElement('div', { key: v.v, style: { position: 'relative', padding: '8px 0 8px 8px' } },
              React.createElement('div', { style: { position: 'absolute', left: -25, top: 14, width: 12, height: 12, borderRadius: '50%', background: isCur ? '#2563eb' : '#fff', border: isCur ? 'none' : '3px solid rgba(128,128,128,.5)', boxSizing: 'border-box' } }),
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
                React.createElement('span', { style: { fontWeight: 700, fontSize: 12.5 } }, 'v' + v.v + ' · ' + (SOURCE_LABEL[v.source] || v.source)),
                React.createElement('span', { style: { fontSize: 11, color: 'var(--dsw-alias-label-secondary, #8493ab)' } }, fmtTime(v.at) + ' · ' + v.lines + ' 行'),
                isCur && React.createElement('span', { style: { fontSize: 10.5, fontWeight: 700, color: '#2563eb', background: 'rgba(37,99,235,.1)', borderRadius: 4, padding: '1px 6px' } }, '当前')),
              React.createElement('div', { style: { display: 'flex', gap: 6, marginTop: 5, flexWrap: 'wrap' } },
                btn('查看', () => onViewVersion(v.v), { ghost: true, style: { padding: '2px 8px' } }),
                !isCur && btn('对比当前↔v' + v.v, () => onCompare(v.v), { style: { padding: '2px 8px' } }),
                !isCur && btn('回滚到此', () => onRollback(v.v), { style: { padding: '2px 8px' } }),
                btn('下载', () => onDownload(v.v), { ghost: true, style: { padding: '2px 8px' } })))
          })),
        preview && React.createElement('div', { style: { marginTop: 8 } }, preview))
    }

    // ===== 粘贴对话框 =====
    function PasteDialog(props: any): any {
      const { onConfirm, onCancel, name, onName, code, onCode } = props
      return React.createElement('div', { style: { border: '1px solid rgba(37,99,235,.35)', borderRadius: 10, padding: 12, margin: '8px 0', background: 'rgba(37,99,235,.04)' } },
        React.createElement('div', { style: { fontWeight: 700, fontSize: 12.5, marginBottom: 6 } }, '+ 粘贴代码'),
        React.createElement('input', { value: name, onChange: (e: any) => onName(e.target.value), placeholder: '文件名（如 snippet-1.py，留空自动命名）', style: { width: '100%', boxSizing: 'border-box', fontSize: 12, padding: '5px 8px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', marginBottom: 6, background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' } }),
        React.createElement('textarea', { value: code, onChange: (e: any) => onCode(e.target.value), placeholder: '在此粘贴代码…', rows: 6, spellCheck: false, style: { width: '100%', boxSizing: 'border-box', fontSize: 12, lineHeight: '18px', fontFamily: 'ui-monospace, Consolas, monospace', padding: '6px 8px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', marginBottom: 8, resize: 'vertical', background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' } }),
        React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end' } },
          btn('取消', onCancel, { style: { padding: '3px 10px' } }),
          btn('创建文件', onConfirm, { primary: true, style: { padding: '3px 10px' }, disabled: code.trim() === '' })))
    }

    // ===== AI 助手条 =====
    interface AiProviderInfo { id: string; displayName: string; baseURL: string; apiKeyEnv: string; keyAvailable: boolean; models: Array<{ id: string; name?: string }> }

    function AiAssistant(props: any): any {
      ...
      const provider = providers.find((p: AiProviderInfo) => p.id === providerId)
      const keyOk = provider?.keyAvailable || false
      return React.createElement('div', { style: { borderTop: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0 } },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', flexWrap: 'wrap' } },
          React.createElement('span', { style: { fontSize: 11, fontWeight: 700, color: 'var(--dsw-alias-label-secondary, #8493ab)', flexShrink: 0 } }, '🤖 AI'),
          providers.length === 0
            ? React.createElement('span', { style: { fontSize: 11, color: '#b45309' } }, '未检测到可用 API Key')
            : React.createElement('select', {
                value: aiKey,
                onChange: (e: any) => onKey(e.target.value),
                ...
                providers.map((p: AiProviderInfo) => React.createElement('optgroup', { key: p.id, label: p.displayName + (p.keyAvailable ? '' : '（未配置 Key）') },
                  (p.models.length > 0 ? p.models : [{ id: '' }]).map((m: any) => React.createElement('option', { key: p.id + '|' + m.id, value: p.id + '|' + m.id, disabled: !p.keyAvailable }, m.name || m.id)))),
              ),
          React.createElement('input', {
            value: input,
            onChange: (e: any) => onInput(e.target.value),
            onKeyDown: (e: any) => { if (e.key === 'Enter' && !e.shiftKey && keyOk && !busy) onChat() },
          ...
          btn(busy ? '…' : '对话', onChat, { primary: true, disabled: !keyOk || busy || input.trim() === '', style: { padding: '3px 10px' } }),
          btn('修改选中(' + props.selectedCount + ')', onApply, { disabled: !keyOk || busy || input.trim() === '' || props.selectedCount === 0, style: { padding: '3px 10px' } }),
          React.createElement('label', { style: { fontSize: 10.5, color: 'var(--dsw-alias-label-secondary, #8493ab)', cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 3 } },
            React.createElement('input', { type: 'checkbox', checked: attach, onChange: (e: any) => onAttach(e.target.checked), style: { margin: 0 } }), '附文件'),
          btn(expanded ? '▾ 收起' : '▸ 记录', onToggle, { ghost: true, style: { padding: '2px 8px' } }),
        ),
        (notice || expanded) && React.createElement('div', { className: 'cw-scroll', style: { maxHeight: 170, overflow: 'auto', padding: '0 10px 6px', fontSize: 12, lineHeight: '18px' } },
          notice && React.createElement('div', { style: { color: '#16a34a', marginBottom: 4, whiteSpace: 'pre-wrap' } }, notice),
          expanded && messages.length === 0 && React.createElement('div', { style: { color: 'var(--dsw-alias-label-secondary, #8493ab)' } }, '对话记录会显示在这里（勾选「附文件」可把选中文件作为上下文）。'),
          expanded && messages.map((m: any, i: number) => React.createElement('div', { key: i, style: { margin: '3px 0', whiteSpace: 'pre-wrap', wordBreak: 'break-word' } },
            React.createElement('span', { style: { fontWeight: 700, color: m.role === 'user' ? '#2563eb' : '#16a34a', marginRight: 6 } }, m.role === 'user' ? '你' : 'AI'),
    ...
    function WorkbenchPanel(props: any): any {
      ...
      const [verCache, setVerCache] = React.useState<Record<string, string>>({})
      const [compare, setCompare] = React.useState<{ leftText: string; rightText: string; leftLabel: string; rightLabel: string } | null>(null)
      const [verPreview, setVerPreview] = React.useState<{ v: number; content: string } | null>(null)
      ...
      // AI 助手状态
      const [aiProviders, setAiProviders] = React.useState<AiProviderInfo[]>([])
      const [aiKey, setAiKey] = React.useState('')
      const [aiInput, setAiInput] = React.useState('')
      const [aiBusy, setAiBusy] = React.useState(false)
      const [aiMessages, setAiMessages] = React.useState<Array<{ role: string; content: string }>>([])
      ...
      const refreshFull = React.useCallback(async () => {
        try {
          ...
          const list = Array.isArray(data.files) ? (data.files as any[]) : []
          setFiles(list)
          setSelected((prev) => {
          ...
          setActiveId((prev) => (prev && list.some((f: any) => f.id === prev) ? prev : (list[0]?.id ?? null)))
          return true
        } catch { return false }
      }, [])

      const refreshMeta = React.useCallback(async (): Promise<any[] | null> => {
        try {
        ...
        fetch('/dsh-code-workbench/ai/providers')
          ...
          .then((data) => {
            if (data && data.ok && Array.isArray(data.providers)) {
              ...
              const first = data.providers.find((p: AiProviderInfo) => p.keyAvailable && p.models.length > 0)
              if (first) {
                setAiKey((prev) => {
                  if (prev && data.providers.some((p: AiProviderInfo) => prev.startsWith(p.id + '|'))) return prev
      ...
      // ---- 上传 / 文件夹 ----
      const uploadFile = React.useCallback(async (file: File, relPath?: string) => {
        setBusy((n) => n + 1)
        try {
      ...
      const uploadAll = React.useCallback(async (fileList: File[] | ArrayLike<File>, relPaths?: string[]) => {
      ...
      const onPickDir = (e: any) => {
        const fl = e.target && e.target.files
        if (fl && fl.length > 0) {
          const list = Array.from(fl as File[])
          const keep: File[] = []
          const rels: string[] = []
          for (const f of list) {
            ...
            if (segs.some((s) => s.startsWith('.') || SKIP_DIR_PARTS.includes(s))) continue
      ...
      const doPaste = async () => {
        ...
        try {
          ...
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || '粘贴失败')
          setPasteOpen(false); setPasteName(''); setPasteCode('')
          await refreshFull()
      ...
      const openTab = (id: string) => {
        ...
        setActiveId(id); setEditing(false); setView('source')
      }
      const closeTab = (id: string, e: any) => {
        ...
        setTabs((prev) => {
          ...
          if (activeId === id) {
            ...
            setActiveId(fallback); setEditing(false); setView('source')
          }
          return next
        })
      }
      ...
      const saveEdit = async () => {
        ...
        try {
          ...
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || '保存失败')
          setEditing(false)
          await refreshFull()
          setView('source')
        } catch (e) {
      ...
      // ---- 历史 ----
      const fetchVersion = async (id: string, v: number): Promise<string | null> => {
        const key = id + ':' + v
        if (verCache[key]) return verCache[key]
        try {
          const res = await fetch('/dsh-code-workbench/version/' + encodeURIComponent(id) + '/' + v)
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || '读取版本失败')
      ...
      const viewVersion = async (v: number) => {
        ...
        if (content != null) setVerPreview({ v, content })
      }
      const compareWithCurrent = async (v: number) => {
        if (!activeFile) return
      ...
      const doRollback = async (v: number) => {
        if (!activeFile) return
        if (!window.confirm(`回滚 ${activeFile.name} 到 v${v}？将追加一条 rollback 记录（历史不销毁）。`)) return
        try {
          ...
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || '回滚失败')
          await refreshFull()
          setView('history')
        } catch (e) {
          setError(e instanceof Error ? e.message : String(e))
      ...
      const downloadVersion = (v: number) => {
        if (!activeFile) return
      ...
      const aiContextSystem = (): string => {
        if (!aiAttach || selectedFiles.length === 0) return '你是代码工作台的内置 AI 助手。用户可能附带代码上下文。'
      ...
      const aiChat = async () => {
        if (!currentAi || aiInput.trim() === '') return
        ...
        try {
          ...
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || 'AI 调用失败')
      ...
      const aiApply = async () => {
        if (!currentAi || aiInput.trim() === '' || selectedFiles.length === 0) return
        ...
        try {
          ...
          const data = await res.json().catch(() => null)
          if (!res.ok || !data || data.ok !== true) throw new Error((data && data.error) || 'AI 修改失败')
          await refreshFull()
          const okList = (data.applied || []).filter((a: any) => a.ok)
          setAiNotice(`✅ 已修改 ${okList.length} 个文件，生成新版本：\n${okList.map((a: any) => '· ' + a.name + (a.summary ? ' — ' + a.summary : '')).join('\n')}\n${data.overview ? '\n' + data.overview : ''}`)
          setAiExpanded(true)
        } catch (e) {
          setAiNotice('❌ ' + (e instanceof Error ? e.message : String(e)))
        } finally {
      ...
      const renderEditor = () => {
        if (!activeFile) return hint('在左侧选择或打开一个文件')
        if (compare && view === 'diff') {
          return React.createElement('div', null,
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, marginBottom: 6 } },
              React.createElement('span', { style: { fontWeight: 700 } }, compare.leftLabel + ' ↔ ' + compare.rightLabel),
              React.createElement('span', { style: { flex: 1 } }),
              btn('退出对比', () => setCompare(null), { ghost: true, style: { padding: '2px 8px' } })),
            React.createElement(DiffView, { leftText: compare.leftText, rightText: compare.rightText, mode: diffMode, showFold: true }))
        }
        if (view === 'source') {
        ...
        if (view === 'diff') {
          if (compare) return React.createElement(DiffView, { leftText: compare.leftText, rightText: compare.rightText, mode: diffMode, showFold: true })
          if (activeFile.current <= 1) return hint('该文件尚无修改（提交 AI 处理、面板 AI「修改选中」、或手动编辑保存后，即可在此对比 Diff）')
        ...
        if (view === 'history') {
          return React.createElement(HistoryView, {
            ...
            preview: verPreview ? React.createElement('div', null,
              React.createElement('div', { style: { fontSize: 11.5, color: 'var(--dsw-alias-label-secondary, #8493ab)', marginBottom: 4 } }, 'v' + verPreview.v + ' 内容预览'),
              React.createElement(LineCode, { code: verPreview.content, lang: activeFile.lang, maxHeight: 240 })) : null,
          })
        }
        return null
      }
      ...
      return React.createElement('div', {
        ...
        // ===== 顶栏 =====
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', borderBottom: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0 } },
          React.createElement('span', { style: { fontWeight: 700, fontSize: 13, whiteSpace: 'nowrap' } }, '💻 代码工作台'),
          React.createElement('span', { style: { fontSize: 11, color: 'var(--dsw-alias-label-secondary, #8493ab)', background: 'var(--dsw-specific-input-major, #fff)', border: '1px solid rgba(128,128,128,.2)', borderRadius: 999, padding: '0 9px', whiteSpace: 'nowrap' } },
            files.length + ' 文件 · ' + modifiedCount + ' 已改'),
          loading && React.createElement('span', { style: { fontSize: 11, color: '#b45309', background: 'rgba(180,83,9,.1)', borderRadius: 999, padding: '0 9px', whiteSpace: 'nowrap' } }, '⏳ 处理中'),
          React.createElement('span', { style: { flex: 1 } }),
          selectedFiles.length > 0 && btn('⬇ 下载(' + selectedFiles.length + ')', () => downloadUrl('/dsh-code-workbench/download-all?ids=' + selectedFiles.map((f) => f.id).join(','), 'code-workbench.zip'), { style: { padding: '2px 9px', fontSize: 11 } }),
          btn(maximized ? '⤡' : '⤢', () => setMaximized(!maximized), { ghost: true, style: { padding: '2px 7px', fontSize: 13 }, title: maximized ? '还原' : '最大化' }),
          React.createElement('button', { type: 'button', onClick: () => setOpen(false), title: '关闭 (Esc)', style: { border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 15, lineHeight: 1, color: 'var(--dsw-alias-label-secondary, #8493ab)', padding: '2px 6px' } }, '×')),
        // ===== 主区 =====
        React.createElement('div', { style: { display: 'flex', flex: 1, minHeight: 0 } },
          // 文件栏
          React.createElement('div', { style: { width: sidebarWidth, flexShrink: 0, borderRight: '1px solid rgba(128,128,128,.15)', display: 'flex', flexDirection: 'column', minHeight: 0 } },
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px 3px' } },
              React.createElement('span', { style: { fontSize: 10.5, fontWeight: 700, color: 'var(--dsw-alias-label-secondary, #8493ab)', letterSpacing: '.08em' } }, '文件'),
              React.createElement('span', { style: { display: 'flex', gap: 4, alignItems: 'center' } },
                React.createElement('span', { onClick: toggleSelectAll, title: '全选/全不选', style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary, #8493ab)', cursor: 'pointer', background: 'rgba(128,128,128,.07)', borderRadius: 4, padding: '0 6px' } }, '全选'),
                React.createElement('span', { style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary, #8493ab)', background: 'rgba(128,128,128,.07)', borderRadius: 4, padding: '0 6px' } }, '已选 ' + selected.size + '/' + files.length))),
            React.createElement('input', {
              ref: searchRef,
              value: search,
            ...
            React.createElement('div', { style: { display: 'flex', gap: 3, padding: '0 10px', marginBottom: 5, flexWrap: 'wrap' } },
              langs.map((l) => React.createElement('span', {
                key: l,
              ...
              }, l === 'all' ? '全部' : (l + ' ' + langCounts[l])))),
            React.createElement('div', { className: 'cw-scroll', style: { flex: 1, overflow: 'auto', padding: '0 6px' } },
              shownFiles.length === 0 && hint(files.length === 0 ? '拖入 / 选择文件 / 📁 文件夹' : '无匹配文件'),
              shownFiles.map((f: any) => {
                ...
                return React.createElement('div', { key: f.id, onClick: () => openTab(f.id), title: f.name, style: Object.assign({ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 7px', borderRadius: 7, cursor: 'pointer' }, isActive ? { background: 'rgba(37,99,235,.09)' } : {}) },
                  React.createElement('span', {
                    onClick: (e: any) => { e.stopPropagation(); toggleSelect(f.id) },
                    style: {
                  ...
                  React.createElement('span', { className: 'cw-tag-lang' }, LANG_TAG[f.lang] || f.lang.slice(0, 2)),
                  React.createElement('div', { style: { flex: 1, minWidth: 0 } },
                    React.createElement('div', { style: { fontSize: 12, fontWeight: isActive ? 600 : 400, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, base),
                    React.createElement('div', { style: { fontSize: 9.5, color: 'var(--dsw-alias-label-secondary, #8493ab)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } },
                      dir ? (dir + ' · ' + f.lines + ' 行') : (f.lang + ' · ' + f.lines + ' 行'))),
                  badge)
              })),
            React.createElement('div', { style: { padding: '7px 8px 8px', display: 'flex', gap: 5, borderTop: '1px solid rgba(128,128,128,.15)', flexShrink: 0 } },
              btn('＋粘贴', () => setPasteOpen(!pasteOpen), { style: { flex: 1, justifyContent: 'center', padding: '4px 2px', fontSize: 11 }, active: pasteOpen }),
              React.createElement('label', { style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(128,128,128,.35)', borderRadius: 7, padding: '4px 2px', cursor: busy > 0 ? 'wait' : 'pointer', fontSize: 11, color: 'var(--dsw-alias-label-primary, #1e293b)', whiteSpace: 'nowrap' } },
                busy > 0 ? '上传中…' : '＋文件',
                React.createElement('input', { type: 'file', multiple: true, accept: ACCEPT, onChange: onPick, style: { display: 'none' } })),
              React.createElement('label', { style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(128,128,128,.35)', borderRadius: 7, padding: '4px 2px', cursor: 'pointer', fontSize: 11, color: 'var(--dsw-alias-label-primary, #1e293b)', whiteSpace: 'nowrap' }, title: '选择文件夹（自动过滤 .git/node_modules/二进制 等）' },
                '📁 文件夹',
                React.createElement('input', { type: 'file', webkitdirectory: '', directory: '', multiple: true, onChange: onPickDir, style: { display: 'none' } }))),
            pasteOpen && React.createElement(PasteDialog, {
              onConfirm: doPaste,
          ...
          // 编辑区 + AI 助手
          React.createElement('div', { style: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 } },
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0, overflowX: 'auto', maxWidth: '100%' } },
              tabs.length === 0 && React.createElement('span', { style: { padding: '6px 12px', fontSize: 11.5, color: 'var(--dsw-alias-label-secondary, #8493ab)' } }, '未打开文件'),
              tabs.map((id) => {
                ...
                return React.createElement('div', {
                  ...
                  dirty ? React.createElement('span', { style: { width: 6, height: 6, borderRadius: '50%', background: '#b45309', display: 'inline-block' } }) : null,
                  (f.name.split('/').pop() || f.name),
                  React.createElement('span', { onClick: (e) => closeTab(id, e), style: { color: 'var(--dsw-alias-label-secondary, #8493ab)', padding: '0 2px', cursor: 'pointer', fontSize: 12 }, title: '关闭' }, '✕'))
              })),
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderBottom: '1px solid rgba(128,128,128,.15)', flexShrink: 0, flexWrap: 'wrap' } },
              btn('源码', () => { setView('source'); setCompare(null) }, { ghost: !(view === 'source'), active: view === 'source' && !compare, style: { padding: '3px 10px' } }),
              btn(activeFile && activeFile.current > 1 ? ('Diff +' + currentDiffStats.add + ' −' + currentDiffStats.del) : 'Diff', () => { setView('diff'); setCompare(null) }, { ghost: !(view === 'diff' && !compare), active: view === 'diff' && !compare, style: { padding: '3px 10px' } }),
              btn('报告', () => { setView('report'); setCompare(null) }, { ghost: !(view === 'report'), active: view === 'report', style: { padding: '3px 10px' } }),
              btn('历史', () => { setView('history'); setCompare(null) }, { ghost: !(view === 'history'), active: view === 'history', style: { padding: '3px 10px' } }),
              activeFile && activeFile.current > 1 && view === 'diff' && !compare && React.createElement('span', { style: { display: 'inline-flex', overflow: 'hidden', border: '1px solid rgba(128,128,128,.3)', borderRadius: 6, marginLeft: 4 } },
                React.createElement('span', { onClick: () => setDiffMode('split'), style: { padding: '2px 9px', fontSize: 10.5, cursor: 'pointer', background: diffMode === 'split' ? 'rgba(128,128,128,.1)' : 'transparent', color: 'var(--dsw-alias-label-primary, #1e293b)' } }, '并排'),
                React.createElement('span', { onClick: () => setDiffMode('unified'), style: { padding: '2px 9px', fontSize: 10.5, cursor: 'pointer', background: diffMode === 'unified' ? 'rgba(128,128,128,.1)' : 'transparent', color: 'var(--dsw-alias-label-primary, #1e293b)' } }, '统一'))),
            React.createElement('div', { className: 'cw-scroll', style: { flex: 1, minHeight: 0, overflow: 'auto', padding: '8px 10px' } },
              error && React.createElement('div', { style: { color: '#dc2626', marginBottom: 8, fontSize: 12 } }, '操作失败：' + error),
              renderEditor()),
        ...
        // ===== 命令栏（宿主 agent 流程） =====
        React.createElement('div', { style: { borderTop: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0, padding: '6px 10px' } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' } },
            React.createElement('span', { style: { fontSize: 10.5, color: 'var(--dsw-alias-label-secondary, #8493ab)', marginRight: 2, flexShrink: 0 } }, '操作:'),
            ACTIONS.map((a) => btn(a.label, () => setAction(a.id), { active: action === a.id, style: { padding: '2px 8px', fontSize: 11 } })),
            action === 'convert' && React.createElement('select', { value: targetLang, onChange: (e: any) => setTargetLang(e.target.value), style: { fontSize: 11, padding: '2px 5px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' } },
          ...
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 } },
            ...
            btn(selectedFiles.length > 0 && action ? ('⌘⏎ 提交 · ' + selectedFiles.length + ' 个') : '⌘⏎ 提交', submit, { primary: true, disabled: !canSubmit(), style: { padding: '4px 12px' } }),
            btn('插入对话', insertAndSend, { ghost: true, style: { padding: '2px 8px' }, disabled: !inputActions || selectedFiles.length === 0 })),
        ),
      )
    }

    const apply = (ctx: any): void => {
      ensureStyles()
      ctx.effect(
