...
export function apply(ctx) {
    ...
    const tool = {
        ...
        async execute(args) {
            ...
            if (action === 'read') {
                if (!args?.fileId)
                    throw new Error('code_workbench read 需要 fileId 参数（可先用 action="list" 查看）');
                const record = getFile(args.fileId);
                if (!record)
                    throw new Error(`找不到代码文件 ${args.fileId}（请先通过 web 面板「💻 代码」上传，或用 action="list" 查看）`);
            ...
            if (action === 'modify') {
                if (!args?.fileId)
                    throw new Error('code_workbench modify 需要 fileId 参数');
                if (typeof args?.modification !== 'string' || args.modification.trim() === '')
                    throw new Error('code_workbench modify 需要 modification 参数（修改后的完整代码）');
            ...
            if (action === 'review') {
                if (!args?.fileId)
                    throw new Error('code_workbench review 需要 fileId 参数');
                if (typeof args?.report !== 'string' || args.report.trim() === '')
                    throw new Error('code_workbench review 需要 report 参数（Markdown 评测报告）');
