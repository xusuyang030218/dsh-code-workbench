/**
 * web 半：同源路由，供浏览器面板上传/粘贴/编辑/版本/下载 + AI 直连。
 * 独立成行（而非并入 host 工具半）是因为 cordis 的 inject 是强依赖：
 * webServer 只在 web profile 存在，headless 下挂载本行会永远 PENDING。
 */
import { MAX_FILE_BYTES, detectLang, countLines, decodeText, looksBinaryDecoded, buildZip, } from './parse.js';
import { getFile, listFiles, saveFile, setModified, addManualVersion, rollbackTo, getVersion, } from './store.js';
import { chatCompletions, getApiKey, resolveProviders, applyModifications } from './ai.js';
    ...
    ctx.effect(() => {
        ...
        return webServer.register({
            ...
            async handler(req, res) {
                ...
                try {
                    // --- AI providers 列表（脱敏，不下发 key） ---
                    if (pathname === '/dsh-code-workbench/ai/providers' && req.method === 'GET') {
                    ...
                    // --- AI 直接对话 ---
                    if (pathname === '/dsh-code-workbench/ai/chat' && req.method === 'POST') {
                        const body = await readJsonBody(req);
                        const providerId = String(body?.provider ?? '');
                        const model = String(body?.model ?? '');
                        const messages = Array.isArray(body?.messages) ? body.messages : [];
                        if (!providerId || !model) {
                            json(res, 400, { ok: false, error: '缺少 provider/model 参数' });
                            return;
                        }
                        if (messages.length === 0 || messages.some((m) => !m || typeof m.content !== 'string')) {
                            json(res, 400, { ok: false, error: 'messages 格式不正确' });
                            return;
                        }
                        const providers = await resolveProviders(ctx);
                        const provider = providers.find((p) => p.id === providerId);
                        if (!provider) {
                            json(res, 400, { ok: false, error: `未知 provider：${providerId}` });
                            return;
                        }
                        const apiKey = await getApiKey(ctx, provider.apiKeyEnv);
                        if (!apiKey) {
                            json(res, 400, { ok: false, error: `Provider ${provider.displayName} 的 API Key 不可用（${provider.apiKeyEnv}），请在 DSH 设置中配置` });
                            return;
                        }
                        const reply = await chatCompletions({
                            ...
                            apiKey,
                            model,
                            messages: messages.map((m) => ({ role: String(m.role ?? 'user'), content: String(m.content) })),
                            temperature: body?.temperature !== undefined ? Number(body.temperature) : undefined,
                        });
                        json(res, 200, { ok: true, reply: reply.content, model: reply.model });
                        return;
                    }
                    // --- AI 修改选中文件（写回 ai 版本） ---
                    if (pathname === '/dsh-code-workbench/ai/apply' && req.method === 'POST') {
                        const body = await readJsonBody(req);
                        const providerId = String(body?.provider ?? '');
                        const model = String(body?.model ?? '');
                        ...
                        if (fileIds.length === 0) {
                            json(res, 400, { ok: false, error: '请先勾选要修改的文件' });
                            return;
                        }
                        if (instruction === '') {
                            json(res, 400, { ok: false, error: '修改指令不能为空' });
                            return;
                        }
                        const providers = await resolveProviders(ctx);
                        const provider = providers.find((p) => p.id === providerId);
                        ...
                        const files = fileIds
                            ...
                            .filter((f) => f != null);
                        if (files.length === 0) {
                            json(res, 400, { ok: false, error: '找不到要修改的文件' });
                            return;
                        }
                        const result = await applyModifications(ctx, {
                            provider,
                            model,
                            files: files.map((f) => ({ id: f.id, name: f.name, lang: f.lang, lines: f.lines, content: f.content })),
                            instruction,
                        });
                        const applied = result.applied.map((m) => {
                            const record = getFile(m.fileId);
                            if (!record)
                                return { fileId: m.fileId, name: m.fileId, ok: false, error: '文件不存在' };
                            setModified(record.id, m.content);
                            return { fileId: record.id, name: record.name, ok: true, summary: m.summary ?? '' };
                        });
                        json(res, 200, { ok: true, applied, overview: result.overview });
                        return;
                    }
                    // --- 上传（原始字节 + 二进制过滤） ---
                    if (pathname === '/dsh-code-workbench/upload' && (req.method === 'POST' || req.method === 'PUT')) {
                        ...
                        if (looksBinaryDecoded(content)) {
                            json(res, 400, { ok: false, error: `「${name}」是二进制文件（或编码无法识别），已跳过` });
                            return;
                        }
                        const record = saveFile({ name, lang: detectLang(name), size: data.byteLength, lines: countLines(content), content, source: 'upload' });
                    ...
                    if (pathname === '/dsh-code-workbench/paste' && (req.method === 'POST' || req.method === 'PUT')) {
                        ...
                        const record = saveFile({ name, lang: detectLang(name), size: Buffer.byteLength(content, 'utf8'), lines: countLines(content), content, source: 'paste' });
                        json(res, 200, { ok: true, file: recordView(record) });
                        return;
                    }
                    // --- 手动编辑保存（追加 manual 版本） ---
                    ...
                    if (rbMatch && (req.method === 'POST' || req.method === 'PUT')) {
                        ...
                        const body = await readJsonBody(req);
                        const record = rollbackTo(fileId, Number(body?.to));
                        json(res, 200, { ok: true, file: recordView(record) });
                        return;
                    }
                    // --- 版本内容 ---
