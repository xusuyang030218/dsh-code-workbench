...
export async function getApiKey(ctx, ref) {
    try {
...
export async function resolveProviders(ctx) {
    const out = [
    ...
    try {
        ...
        const providers = (pi && typeof pi === 'object' && pi.providers) ?? {};
        for (const [id, raw] of Object.entries(providers)) {
            ...
            const models = Array.isArray(raw.models)
                ? raw.models
                    .map((m) => ({ id: String(m?.id ?? ''), name: m?.name ?? m?.id }))
                    .filter((m) => m.id)
                : [];
            out.push({
                id,
...
export async function applyModifications(ctx, opts) {
    const apiKey = await getApiKey(ctx, opts.provider.apiKeyEnv);
    if (!apiKey)
        throw new Error(`API Key 不可用（${opts.provider.apiKeyEnv}），请在 DSH 设置中配置后重试`);
...
export function parseApplyJson(text) {
    const cleaned = text.replace(/^```(?:json)?\s*/i, '').replace(/```\s*$/, '').trim();
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start < 0 || end <= start)
        throw new Error('AI 未返回 JSON 修改清单，请重试');
    ...
    if (mods.length === 0)
        throw new Error('AI 返回的修改清单为空（modifications 缺省）');
    const applied = mods
        .filter((m) => m && typeof m.fileId === 'string' && typeof m.content === 'string')
        .map((m) => ({
        fileId: m.fileId,
        content: m.content,
    ...
    if (applied.length === 0)
        throw new Error('AI 返回的修改清单缺少 fileId/content 字段');
