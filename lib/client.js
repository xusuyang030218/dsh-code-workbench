/**
 * dsh-code-workbench browser 端 — v3 三区工作台（设计文档 outputs/code-workbench-redesign）
 *
 * 布局：顶栏状态胶囊 / 文件栏（勾选 + 搜索 + 粘贴/上传）/ 编辑区（文件 Tab + 四视图）/
 *       命令栏（操作 chips + 要求 + 动态提交按钮）
 *
 * v3 功能：N1 粘贴代码建文件 · N2 勾选圈定提交范围 · N3 源码行号 + 编辑态 ·
 *          N4 版本历史/回滚/对比 · N5 Diff 双模式 + 词级高亮 + 折叠 + 统计徽章 ·
 *          N6 布局重构 · N7 Markdown 报告渲染 · N8 按勾选打包下载 ·
 *          N9 /list?meta=1 轻量轮询 · N10 面板最大化 · N11 快捷键 · N13 复制带行号
 *
 * 零外部依赖：语法高亮自写 token 化器、diff 自写 LCS（行级 + 字符级）、CSS 内联注入。
 * 装载链：官方 client-modules 扫描 dsh.client 声明后以 /plugins/dsh-code-workbench/client.js
 * 供给浏览器（window.__ModuleLoader__.load 契约）。React 由平台 require 提供。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ACTIONS = [
      ...
      { id: 'fix', label: '修Bug', icon: '🐛' },
      { id: 'refactor', label: '重构', icon: '🔧' },
      { id: 'optimize', label: '性能', icon: '⚡' },
      { id: 'comment', label: '注释', icon: '📝' },
      { id: 'test', label: '测试', icon: '🧪' },
      { id: 'convert', label: '转语言', icon: '🔄' },
    ]
    ...
    /** 行级 diff → rows（same/mod/del/add）；mod = 相邻 del+add 等长配对。 */
    ...
    /** 词级 diff：对修改对（单行）跑字符级 LCS，返回左右两侧 span 序列。 */
    function wordDiffSpans(leftLine, rightLine) {
      const a = Array.from(leftLine)
      const b = Array.from(rightLine)
      const ops = lcsOps(a, b)
      const fallback = { left: [{ text: leftLine, type: 'same' }], right: [{ text: rightLine, type: 'same' }] }
    ...
    /** 折叠：连续 same 行超过 threshold → 占位（保留前后各 ctx 行）。 */
    function foldSame(rows, ctx, threshold) {
      ctx = ctx || 3
    ...
    function renderMarkdown(md) {
      ...
      const flushTable = () => {
        ...
        out.push(React.createElement('table', { key: 't' + key, style: { borderCollapse: 'collapse', fontSize: 12, margin: '6px 0' } },
          React.createElement('thead', null, React.createElement('tr', null, head.map((c, i) => React.createElement('th', { key: i, style: { border: '1px solid rgba(128,128,128,.25)', padding: '3px 8px', textAlign: 'left' } }, c)))),
          React.createElement('tbody', null, tableBuf.slice(1).map((row, ri) => React.createElement('tr', { key: ri }, row.map((c, ci) => React.createElement('td', { key: ci, style: { border: '1px solid rgba(128,128,128,.25)', padding: '3px 8px' } }, c))))),
        ))
        tableBuf = []
      }
    ...
    const CSS_TEXT = [
      ...
      '.cw-mdc { font-family: ui-monospace, "Cascadia Code", Consolas, monospace; font-size: .92em; background: rgba(37,99,235,.1); color: #2563eb; padding: 0 3px; border-radius: 3px; }',
      ...
      '.cw-lang { font-family: ui-monospace, Consolas, monospace; font-size: 10px; font-weight: 700; border-radius: 4px; padding: 1px 5px; background: rgba(37,99,235,.1); color: #2563eb; }',
      '.cw-tag-lang { font-family: ui-monospace, Consolas, monospace; font-size: 10px; font-weight: 700; border-radius: 4px; padding: 1px 6px; letter-spacing: .03em; }',
    ...
    function CodeButton() {
      ...
      return React.createElement('button', {
        type: 'button',
        title: '代码工作台（上传/粘贴代码 → AI 修改 → 下载 + diff + 版本历史）',
        'aria-label': '代码工作台',
        onClick: toggle,
        style: {
          border: 'none',
    ...
    function LineCode(props) {
      ...
      return React.createElement('div', { className: 'cw-scroll cw-code', style: { overflow: 'auto', maxHeight: maxHeight || 340, background: 'rgba(128,128,128,.05)', border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, margin: '6px 0' } },
        lines.map((line, i) => React.createElement('div', { key: i, style: { display: 'flex', padding: '0 10px', lineHeight: '18px' } },
          ...
          React.createElement('span', { style: { whiteSpace: 'pre', flex: 1, minWidth: 0, overflow: 'hidden' } }, line === '' ? ' ' : renderHighlighted(line, lang)),
        )),
      )
    }
    ...
    function CodeView(props) {
      ...
      // 编辑态：下层高亮 pre + 上层透明文字 textarea（滚动同步）。
      return React.createElement('div', { style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, background: 'rgba(128,128,128,.05)', overflow: 'hidden' } },
        header,
        React.createElement('div', { style: { display: 'flex', maxHeight: props.maxHeight || 360 } },
          React.createElement('div', { ref: lnRef, className: 'cw-scroll', style: { width: 44, overflow: 'hidden', flexShrink: 0, background: 'rgba(128,128,128,.04)', borderRight: '1px solid rgba(128,128,128,.12)', padding: '8px 0', fontSize: 12, lineHeight: '18px', textAlign: 'right', color: 'rgba(128,128,128,.6)', userSelect: 'none' } },
            lines.map((_, i) => React.createElement('div', { key: i, style: { paddingRight: 8 } }, String(i + 1))),
          ),
          React.createElement('div', { style: { position: 'relative', flex: 1, minWidth: 0 } },
            React.createElement('pre', {
              ref: preRef,
              className: 'cw-code',
              style: Object.assign({}, base, { position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', color: 'var(--dsw-alias-label-primary, #1e293b)' }),
              'aria-hidden': 'true',
            },
            ...
            React.createElement('textarea', {
              ...
              style: Object.assign({}, base, {
                ...
                color: 'transparent',
                caretColor: 'var(--dsw-alias-label-primary, #1e293b)',
                overflow: 'auto',
              }),
            }),
          ),
    ...
    const DIFF_BG = {
      same: 'transparent',
      mod: 'rgba(180, 83, 9, 0.12)',
      add: 'rgba(22, 163, 74, 0.10)',
      del: 'rgba(220, 38, 38, 0.09)',
    }
    function DiffView(props) {
      ...
      const foldKeyOf = (idx) => {
        let key = 0
        for (let i = 0; i < idx; i++) if (folded[i].type === 'folded') key += 1
        return key
      }
      const rowBg = (type) => DIFF_BG[type] || 'transparent'
      const wordSpansCache = {}
      const getWordSpans = (left, right) => {
        const k = left + '\u0001' + right
        if (!wordSpansCache[k]) wordSpansCache[k] = wordDiffSpans(left, right)
        return wordSpansCache[k]
      }
      const renderSpans = (spans, clsDel, clsAdd) => spans.map((s, i) =>
        s.type === 'same'
          ? s.text
          : React.createElement('span', { key: i, className: s.type === 'del' || s.type === 'add' ? (s.type === 'del' ? 'cw-wdel' : 'cw-wadd') : '' }, s.text),
      )
        ...
        return React.createElement('div', { className: 'cw-diff', style: { border: '1px solid rgba(128,128,128,.16)', borderRadius: 8, overflow: 'hidden' } },
          ...
          React.createElement('div', { className: 'cw-scroll', style: { maxHeight: props.maxHeight || 380, overflow: 'auto' } },
            folded.map((r, i) => {
              if (r.type === 'folded') {
                ...
                return React.createElement('div', { key: 'f' + i, className: 'cw-diff-fold' },
                  ...
                  React.createElement('button', {
                    type: 'button',
                    onClick: () => setExpanded((prev) => { const s = new Set(prev); if (s.has(k)) s.delete(k); else s.add(k); return s }),
              ...
              const leftCell = r.type === 'add' ? '' : renderCell(r.left, r.lnL, r.type, 'left', r)
              const rightCell = r.type === 'del' ? '' : renderCell(r.right, r.lnR, r.type, 'right', r)
              return React.createElement('div', { key: i, style: { display: 'flex', background: bg } },
                React.createElement('div', { style: { flex: 1, minWidth: 0, padding: '1px 6px 1px 12px', whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, leftCell),
                React.createElement('div', { style: { flex: 1, minWidth: 0, padding: '1px 6px 1px 12px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', borderLeft: '1px solid rgba(128,128,128,.12)' } }, rightCell),
              )
            }),
          ),
        )
        ...
        React.createElement('div', { className: 'cw-scroll', style: { maxHeight: props.maxHeight || 380, overflow: 'auto', padding: '4px 0' } },
          folded.map((r, i) => {
            ...
            if (r.type === 'mod') {
              ...
              return React.createElement('div', { key: i },
                React.createElement('div', { style: { display: 'flex', padding: '1px 12px', background: rowBg('del'), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
                  ...
                  React.createElement('span', { style: { flex: 1 } }, spans.left.map((s, si) => s.type === 'same' ? s.text : React.createElement('span', { key: si, className: 'cw-wdel' }, s.text))),
                ),
                React.createElement('div', { style: { display: 'flex', padding: '1px 12px', background: rowBg('add'), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
                  ...
                  React.createElement('span', { style: { flex: 1 } }, spans.right.map((s, si) => s.type === 'same' ? s.text : React.createElement('span', { key: si, className: 'cw-wadd' }, s.text))),
                ),
              )
            }
            const prefix = r.type === 'del' ? React.createElement('span', { style: { color: '#dc2626', fontWeight: 700, paddingRight: 6 } }, '−')
              : r.type === 'add' ? React.createElement('span', { style: { color: '#16a34a', fontWeight: 700, paddingRight: 6 } }, '+')
              : null
            ...
            return React.createElement('div', { key: i, style: { display: 'flex', padding: '1px 12px', background: rowBg(r.type), whiteSpace: 'pre-wrap', wordBreak: 'break-all' } },
              ...
              prefix,
              React.createElement('span', { style: { flex: 1 } }, r.type === 'same' ? r.left : (r.left != null ? r.left : (r.right != null ? r.right : ''))),
            )
          }),
        ),
      )

      function renderCell(text, ln, type, side, row) {
        if (text == null) return ''
        if (type === 'mod') {
          const spans = getWordSpans(row.left, row.right)
          const sideSpans = side === 'left' ? spans.left : spans.right
          return React.createElement('span', null,
            React.createElement('span', { style: lnStyle }, ln || ''),
            React.createElement('span', null, sideSpans.map((s, si) => s.type === 'same' ? s.text : React.createElement('span', { key: si, className: side === 'left' ? 'cw-wdel' : 'cw-wadd' }, s.text))),
          )
        }
        return React.createElement('span', null,
          React.createElement('span', { style: lnStyle }, ln || ''),
    ...
    function PasteDialog(props) {
      ...
      return React.createElement('div', { style: { border: '1px solid rgba(37,99,235,.35)', borderRadius: 10, padding: 12, margin: '8px 0', background: 'rgba(37,99,235,.04)' } },
        ...
        React.createElement('input', {
          ...
          placeholder: '文件名（如 snippet-1.py，留空自动命名）',
          style: { width: '100%', boxSizing: 'border-box', fontSize: 12, padding: '5px 8px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', marginBottom: 6, background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' },
        }),
        React.createElement('textarea', {
          value: code,
          ...
          rows: 6,
          spellCheck: false,
          style: { width: '100%', boxSizing: 'border-box', fontSize: 12, lineHeight: '18px', fontFamily: 'ui-monospace, Consolas, monospace', padding: '6px 8px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', marginBottom: 8, resize: 'vertical', background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' },
        }),
    ...
    function WorkbenchPanel(props) {
      ...
      const [compare, setCompare] = React.useState(null) // { leftText, rightText, leftLabel, rightLabel }
      ...
      const refreshFull = React.useCallback(async () => {
        try {
          ...
          setActiveId((prev) => (prev && (data.files || []).some((f) => f.id === prev) ? prev : ((data.files || [])[0]?.id ?? null)))
          return true
        } catch { return false }
      }, [])
      ...
      React.useEffect(() => { if (isOpen) refreshFull() }, [isOpen, refreshFull])

      // 提交后轻量轮询：全部勾选文件 hasModified 后拉全量并停止。
      React.useEffect(() => {
      ...
      // 选择文件夹：webkitdirectory 递归列举，保留相对路径（去掉文件夹根名）。
      ...
      const onPickDir = (e) => {
        ...
        if (fl && fl.length > 0) {
          ...
          const rels = list.map((f) => {
            ...
            return idx >= 0 ? rp.slice(idx + 1) : f.name
          })
          if (list.length > MAX_FOLDER_FILES) {
            setError(`文件夹文件过多（${list.length} 个），仅取前 ${MAX_FOLDER_FILES} 个`)
            uploadAll(list.slice(0, MAX_FOLDER_FILES), rels.slice(0, MAX_FOLDER_FILES))
          } else {
            uploadAll(list, rels)
          }
        }
      ...
      const [verPreview, setVerPreview] = React.useState(null) // { v, content }
      ...
      const diffViewEl = activeFile
        ? React.createElement(DiffView, {
            leftText: compare ? compare.leftText : (activeFile.current > 1 ? activeFile.content : ''),
            rightText: compare ? compare.rightText : (activeFile.current > 1 ? activeFile.modifiedContent : activeFile.content),
            mode: diffMode,
            showFold: true,
          })
        : hint('请选择一个文件查看 Diff')
      ...
      const renderEditor = () => {
        ...
        if (view === 'diff') {
          if (compare) return diffViewEl
          if (activeFile.current <= 1) return hint('该文件尚无修改（提交 AI 处理，或点「编辑」手动修改保存后，即可在此对比 Diff）')
          return diffViewEl
        }
        if (view === 'report') {
        ...
        if (view === 'history') {
          return React.createElement(HistoryView, {
            ...
            preview: verPreview ? React.createElement('div', null,
              ...
              React.createElement(LineCode, { code: verPreview.content, lang: activeFile.lang, maxHeight: 240 }),
            ) : null,
          })
        }
        return null
      ...
      const panelStyle = maximized
        ...
        : { width: 'min(880px, 94vw)', maxHeight: '76vh' }
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', { style: { display: 'flex', flex: 1, minHeight: 0 } },
          ...
          React.createElement('div', { style: { width: sidebarWidth, flexShrink: 0, borderRight: '1px solid rgba(128,128,128,.15)', display: 'flex', flexDirection: 'column', minHeight: 0 } },
            ...
            React.createElement('div', { className: 'cw-scroll', style: { flex: 1, overflow: 'auto', padding: '0 6px' } },
              shownFiles.length === 0 && hint(files.length === 0 ? '拖入 / 选择文件 / 选择文件夹' : '无匹配文件'),
              shownFiles.map((f) => {
                ...
                return React.createElement('div', { key: f.id, onClick: () => openTab(f.id), title: f.name, style: Object.assign({ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 7px', borderRadius: 7, cursor: 'pointer' }, isActive ? { background: 'rgba(37,99,235,.09)' } : {}) },
                  ...
                  React.createElement('span', { className: 'cw-tag-lang', style: { flexShrink: 0, fontSize: 9.5, padding: '0 5px' } }, LANG_TAG[f.lang] || f.lang.slice(0, 2)),
            ...
            React.createElement('div', { style: { padding: '7px 8px 8px', display: 'flex', gap: 5, borderTop: '1px solid rgba(128,128,128,.15)', flexShrink: 0 } },
              ...
              React.createElement('label', { style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(128,128,128,.35)', borderRadius: 7, padding: '4px 2px', cursor: 'pointer', fontSize: 11, color: 'var(--dsw-alias-label-primary, #1e293b)', whiteSpace: 'nowrap' }, title: '选择文件夹（递归添加目录内代码文件）' },
                '📁 文件夹',
                React.createElement('input', { ref: dirRef, type: 'file', webkitdirectory: '', directory: '', multiple: true, onChange: onPickDir, style: { display: 'none' } }),
              ),
            ),
          ...
          React.createElement('div', { style: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 } },
            ...
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0, overflowX: 'auto', maxWidth: '100%' } },
              tabs.length === 0 && React.createElement('span', { style: { padding: '7px 12px', fontSize: 11.5, color: 'var(--dsw-alias-label-secondary, #8493ab)' } }, '未打开文件'),
              tabs.map((id) => {
                ...
                return React.createElement('div', {
                  ...
                  style: {
                    display: 'flex', alignItems: 'center', gap: 5, padding: '7px 10px', fontSize: 12, whiteSpace: 'nowrap', cursor: 'pointer',
        ...
        React.createElement('div', { style: { borderTop: '1px solid rgba(128,128,128,.15)', background: 'rgba(128,128,128,.04)', flexShrink: 0, padding: '6px 10px' } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' } },
            ...
            action === 'convert' && React.createElement('select', { value: targetLang, onChange: (e) => setTargetLang(e.target.value), style: { fontSize: 11, padding: '2px 5px', borderRadius: 6, border: '1px solid rgba(128,128,128,.3)', background: 'var(--dsw-specific-input-major, #fff)', color: 'inherit' } },
